...
apt-get -y update
apt-get -y upgrade
apt-get -y install linux-headers-$(uname -r) build-essential zlib1g-dev libssl-dev libreadline5-dev nfs-common
apt-get clean
...
# Create the vbox user so it doesn't get a stupid gid
